    ...
    if ($request->header('Authorization')) {
        ...
        if ((int)$dataUser->user_role > 1) {
            if ((int)$dataUser->id_user_parent > 0) {
                $saldo = $db->query("SELECT COALESCE((SELECT SUM(amount_credit) FROM `app_journal_finance_" . $dataUser->id_user_parent . "` where status > 1), 0) - COALESCE((SELECT SUM(amount_debet) FROM `app_journal_finance_" . $dataUser->id_user_parent . "` where status > 1), 0) as saldo")->getRow()->saldo;
            } else {

                $saldo = $db->query("SELECT COALESCE((SELECT SUM(amount_credit) FROM `app_journal_finance_" . $dataUser->id_user . "` where status > 1), 0) - COALESCE((SELECT SUM(amount_debet) FROM `app_journal_finance_" . $dataUser->id_user . "` where status > 1), 0) as saldo")->getRow()->saldo;
            }
            $realSaldo = 0;
        } else {
            $saldo = $db->query("SELECT COALESCE((SELECT SUM(amount_credit) FROM `admin_journal_finance` where status > 1), 0) - COALESCE((SELECT SUM(amount_debet) FROM `admin_journal_finance` where status > 1), 0) as saldo")->getRow()->saldo;
            $realSaldo = $db->query("SELECT COALESCE((SELECT SUM(amount_credit) FROM `admin_journal_finance` where status > 1 AND (accounting_type = 1001 OR accounting_type = 2001 OR accounting_type = 3001)), 0) - COALESCE((SELECT SUM(amount_debet) FROM `admin_journal_finance` where status > 1 AND (accounting_type = 4 OR accounting_type = 4002)), 0) as saldo", 0)->getRow()->saldo;
        }
