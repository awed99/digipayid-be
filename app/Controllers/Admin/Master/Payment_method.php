...
class Payment_method extends BaseController
{
    ...
    public function postList()
    {
        ...
        $db = db_connect();
        $builder = $db->table('master_payment_method')->where('(payment_method_id_pg = 3 or payment_method_id_pg = 1)')->where('status_admin', 1)->get()->getResult();
        $db->close();
        $finalData = json_encode($builder);
        echo '{
            "code": 0,
    ...
    public function postUpdate()
    {
        ...
        $dataFinal = $query->where('(payment_method_id_pg = 3 or payment_method_id_pg = 1)')->where('status_admin', 1)->get()->getResult();
        $db->close();
