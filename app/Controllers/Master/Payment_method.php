...
class Payment_method extends BaseController
{
    ...
    public function getList()
    {
        ...
        $builder = $db->table('app_payment_method_' . $user->id_user)
            ...
            ->where('status_admin', 1)->get()->getResult();
        $db->close();
    ...
    public function postList0()
    {
        $request = request();
        $dataPost = $request->getJSON();
        $user = cekValidation0('/master/payment_method/list0');
        $db = db_connect();
        $builder = $db->table('app_payment_method_' . $user->id_user)
            ->join('master_payment_method', 'master_payment_method.id_payment_method = app_payment_method_' . $user->id_user . '.id_payment_method')
            ->where('(payment_method_id_pg = 3 or payment_method_id_pg = 1)')
    ...
    public function postLists()
    {
        ...
        $builder = $db->table('app_payment_method_' . $user->id_user)
            ->join('master_payment_method', 'master_payment_method.id_payment_method = app_payment_method_' . $user->id_user . '.id_payment_method')
            ->where('(payment_method_id_pg = 3 or payment_method_id_pg = 1)')
            ->where('status', 1)->where('status_admin', 1)
            ->get()->getResult();
        $db->close();
        $finalData = json_encode($builder);
        echo '{
    ...
    public function postUpdate()
    {
        ...
        $dataFinal = $query->join('master_payment_method', 'master_payment_method.id_payment_method = app_payment_method_' . $user->id_user . '.id_payment_method')
            ->where('(payment_method_id_pg = 3 or payment_method_id_pg = 1)')
            ->where('status', 1)
            ->where('status_admin', 1)
            ->get()->getResult();
